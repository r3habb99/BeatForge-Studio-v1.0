...
function highlightTrackHeader(trackId) {
    ...
    if (targetHeader) {
        targetHeader.classList.add('ring-2', 'ring-blue-500', 'ring-offset-2', 'ring-offset-gray-900');
    ...
    tracks.forEach((track, trackIndex) => {
        ...
        if (needsCreate || !hDiv.hasAttribute('data-rendered')) {
            hDiv.innerHTML = `
                <div class="flex justify-between items-center mb-2">
                    <span class="font-bold text-sm truncate flex-1 mr-2">${track.name}</span>
                    <div class="flex gap-1 flex-shrink-0">
                        <button class="mute-btn text-xs w-6 h-6 rounded ${track.mute ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-400'}" aria-label="Mute track">M</button>
                        <button class="solo-btn text-xs w-6 h-6 rounded ${track.solo ? 'bg-yellow-500 text-black' : 'bg-gray-700 text-gray-400'}" aria-label="Solo track">S</button>
                    </div>
                </div>
                ...
                <div class="flex gap-2 items-center mb-2">
                     ...
                     <input type="range" class="vol-slider flex-1 min-w-0" min="0" max="1" step="0.05" value="${track.vol}" aria-label="Track volume">
                </div>
                ...
                <div class="flex gap-2 items-center mb-2">
                    <div class="flex items-center gap-1 flex-1 min-w-0">
                        ...
                        <input type="range" class="rev-slider flex-1 min-w-0 fx-send-slider text-blue-400" min="0" max="1" step="0.1" value="${track.rev}" aria-label="Reverb send">
                    </div>
                    <div class="flex items-center gap-1 flex-1 min-w-0">
                        <i class="fas fa-clock text-[10px] text-purple-400 flex-shrink-0" title="Delay"></i>
                        <input type="range" class="del-slider flex-1 min-w-0 fx-send-slider text-purple-400" min="0" max="1" step="0.1" value="${track.del}" aria-label="Delay send">
                    </div>
                </div>
                ...
                <div class="flex gap-2 items-center mb-2">
                    ...
                    <input type="range" class="dist-slider flex-1 min-w-0 fx-send-slider text-orange-400" min="0" max="1" step="0.05" value="${track.distortion || 0}" aria-label="Distortion">
                </div>
            ...
            if (muteBtn) {
                muteBtn.className = `mute-btn text-xs w-6 h-6 rounded ${track.mute ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-400'}`;
            }
            if (soloBtn) {
                soloBtn.className = `solo-btn text-xs w-6 h-6 rounded ${track.solo ? 'bg-yellow-500 text-black' : 'bg-gray-700 text-gray-400'}`;
            }
        }
        ...
        for (let i = 0; i < STEP_COUNT; i++) {
            ...
            if (track.type === 'drum') {
                ...
                btn.className = `step-btn w-10 h-10 md:w-12 md:h-12 rounded-sm transition-all ${track.steps[i] ? track.color : 'bg-gray-800'}`;
            } else {
