...
import { closePianoRoll, clearAllNotes, getActiveSynthId } from '../pianoRoll.js';
...
export function setupShortcutsButton() {
    document.getElementById('showShortcutsBtn').addEventListener('click', showShortcuts);
}
        ...
        switch (e.key) {
            ...
            case 'Escape':
                if (getActiveSynthId() !== null) {
                    closePianoRoll();
                    renderUICallback();
                } else if (document.getElementById('shortcutsModal').classList.contains('open')) {
                    closeShortcuts();
                }
                break;
