 ...
 */
export async function exportToWAV(state, tracks, renderDrumCallback, renderSynthCallback) {
    try {
        ...
        for (let step = 0; step < STEP_COUNT; step++) {
            ...
            tracks.forEach(track => {
                ...
                if (track.type === 'drum' && track.steps[step]) {
                    renderDrumCallback(offlineCtx, track, time, masterGain, reverbNode, delayNode);
                    ...
                    notesHere.forEach(note => {
                        ...
                        renderSynthCallback(offlineCtx, track, note, time, noteDuration, masterGain, reverbNode, delayNode);
                    });
                }
            });
        }
